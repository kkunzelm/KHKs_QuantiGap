...
public class ResultsAndText_gj implements PlugIn, Measurements {
	...
	String readme = "This plug-in extends ResultsTable and TextPanel functions."
			...
			+ "\n  arg==\"install\":  installs plugin options" + GJx.sign;
	...
	public void run(String arg) {
		...
		switch (action) {
			...
			case 'h' : {
				IJ.setColumnHeadings(Analyzer.getResultsTable().getColumnHeadings());
			...
			case 's' : {
				new ResultsTable_Image("put");
				break;
			}
			case 't' : {
				new ResultsTable_Image("get");
				// new ResultsList("list");
				break;
			}
			case 'a' : {
				if (!IJ.showMessageWithCancel("About ResultsAndText_gj", readme))
					return;
				break;
			}
			case 'd' : {
			...
			case 'x' : {
				GJx.writeAbout("About ResultsAndText_gj", readme, false);
				break;
			}
			case 'i' : {
				if (!IJ.showMessageWithCancel("ResultsAndText_gj", "InstallPluginsGJ"))
					return;
				(new InstallPluginsGJ()).run("");
				break;
			}
			case 'u' : {
				if (!IJ.showMessageWithCancel("ResultsAndText_gj", "UNInstallPluginsGJ"))
					return;
			...
			case '0' : {
				...
				if (!IJ.showMessageWithCancel("About ResultsAndText_gj", readme))
					return;
				if (!IJ.showMessageWithCancel("ResultsAndText_gj", "ResultsAndText\n Demonstration\n"
...
class ResultsList {
	...
	ResultsList(int options) {
		this("", options);
	}

	ResultsList(String arg) {
		this(arg, -1);
	}

	ResultsList(String arg, int options) {
		...
		if (rt == null)
			if (!IJ.showMessageWithCancel("setup", "null ResultsTable?"))
				return;
		rCount = rt.getCounter();
		if (rCount == 0)
			if (!IJ.showMessageWithCancel("setup", "empty ResultsTable?"))
				return;
		if ((this.options >= 0) && (this.options & FROMFIRST) != 0) {
			to_res = 1;
			from_res = rCount;
		} else {
			to_res = rCount;
	...
	String getRowAsString(ResultsTable rt, int[] colIs, int r) {
		String s = "" + (r + 1);
		for (int i = 0; i < colIs.length; i++)
			if (dCols[i])
				s += "\t" + IJ.d2s(rt.getValue(colIs[i], r), Analyzer.precision);
		return s;
	}
...
class ResultsTable_Image {
	...
	public void run(String arg) {
		rt = Analyzer.getResultsTable();
		if (rt == null)
			IJ.showMessageWithCancel("setup", "null ResultsTable?");
		rCount = rt.getCounter();
	...
	int[] getColumns(ResultsTable rt) {
		int[] cols = new int[nCols(rt)];
		int n = 0;
		for (int i = 0; i < ResultsTable.MAX_COLUMNS; i++) {
			float[] col = rt.getColumn(i);
			if (col != null)
	...
	public boolean put() {
		...
		int nc = nCols(rt);
		if (nCols != nc)
			IJ.showMessageWithCancel("putResultsTable_Image", "" + nCols + " vs " + nc);
		...
		if (array == null)
			if (!IJ.showMessageWithCancel("ResultsTable", "no array?"))
				return false;
	...
	public boolean get() {
		...
		if (imp.getType() != ImagePlus.GRAY32)
			if (!IJ.showMessageWithCancel("ResultsTable", "not GRAY32"))
				return false;
		if (rCount != 0)
			if (!IJ.showMessageWithCancel("ResultsTable", "overwrite " + rCount + " results with ResultsTable_image?"))
				return false;
		rt.reset();
		...
		if (array[array.length - 2] != 1)
			if (!IJ.showMessageWithCancel("getResultsTable_Image", "written by different version"))
				return false;// last row tag
		...
		for (int i = 0; i < nCols; i++) {
			...
			for (int j = 0; j < colL[i]; j++)
				fCHdr[j] = array[(rCount + 2 + j) * nCols + i];
		...
		for (int y = 0; y < rCount; y++) {
			...
			for (int x = 0; x < nCols; x++)
				if (array[yb + x] != -808080)
					rt.setValue(x, y, array[yb + x]);
		}
...
class GJx {
	...
	public static void writeAbout(String title, String readme) {
		writeAbout(title, readme, true);
	}
 ...
 *
 * @param plugin
 *            the plugin name (e.g. "Inverter_", "Inverter_("about")")
 * @param menuCode
 *            PLUGINS_MENU, IMPORT_MENU or SAVE_AS_MENU
 * @param command
 *            the menu item label
 * @param command
...
class InstallPluginsGJ {
	...
	public void run(String arg) {
		if (arg == "about") {
			IJ.showMessage("Install_GJ_Plugins", "all is Trivial");
			return;
		}
		String[][] ps = {{"-", "-", ""}, {"ResultsAndText_gj(\"reset\")", "Results reset", "F1"},
		...
		String[] rets = {"NORMAL_RETURN", "COMMAND_IN_USE", "INVALID_SHORTCUT", "SHORTCUT_IN_USE", "NOT_INSTALLED",
				"COMMAND_NOT_FOUND"};
